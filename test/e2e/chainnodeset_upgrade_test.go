		...
		for _, app := range apps.All() {
			...
			for _, upgradeTest := range app.UpgradeTests {
				...
				It(testName+" ["+app.Name+"]", WithNs(func(ns *corev1.Namespace) {
					...
					// Wait for ChainNodeSet to be running and reach a stable height
					By("Waiting for ChainNodeSet to start with initial version")
					WaitForChainNodeSetHeight(chainNodeSet, 10)

					// Get the validator ChainNode name (it's the same as the ChainNodeSet name)
					RefreshChainNodeSet(chainNodeSet)

					// Get current height and schedule upgrade for height + 30
